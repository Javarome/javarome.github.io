...
import((`./ResumeMessages_${lang}.mjs`)).then(imported => {
  ...
  const famicityApp = new Experience(famicityExp, famicityExp.startDate, famicityExp.endDate, projectsMessages.famicityApp, rueMauriceThorez, [skills.ts, skills.angular, skills.angularJS, skills.sass])
  ...
  const mlPipeline = new Experience(zelrosExp, kmApp.startDate, zelrosExp.endDate, projectsMessages.mlPipeline, rueDeBezons, [skills.ts, skills.fastify, skills.nodejs, skills.python, skills.redis, skills.azure])
  ...
  const beamWebServer = new Experience(beamExp, beamExp.startDate, beamExp.endDate, projectsMessages.beamWebServer, rueDeBezons, [skills.ts, skills.fastify, skills.nodejs, skills.rest, skills.gcp, skills.gitlab])
  ...
  const arn = new Experience(arianeeExp, new Date(2023, 1 - 1, 1), new Date(2023, 4 - 1, 1), projectsMessages.arn, rueDeBezons, [skills.angular, skills.nx, skills.nodejs])
  const dapp = new Experience(arianeeExp, new Date(2023, 4 - 1, 1), arianeeExp.endDate, projectsMessages.dapp, rueDeBezons, [skills.angular, skills.nx, skills.nodejs, skills.gcp])
